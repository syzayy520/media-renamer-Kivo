// PathExistsChecker 测试
// 职责：测试路径存在检查

use app_lib::rename::path_exists_checker::check_path_exists;
...
#[test]
fn test_path_exists_detected() {
    let item = make_preview_item(
        "C:\\test\\file.mkv",
        "C:\\test\\file.mkv", // 目标与源相同
    );
    let conflict = check_path_exists(&item);
    assert!(conflict.is_some());
    let conflict = conflict.unwrap();
    assert_eq!(conflict.conflict_type, ConflictType::TargetExists);
