// PathLengthChecker 测试
// 职责：测试路径长度检查

use app_lib::rename::path_length_checker::check_path_length;
...
#[test]
fn test_path_too_long_detected() {
    let long_path = format!("C:\\test\\{}.mkv", "a".repeat(260));
    let item = make_preview_item("C:\\test\\file.mkv", &long_path);
    let conflict = check_path_length(&item);
    assert!(conflict.is_some());
    let conflict = conflict.unwrap();
    assert_eq!(conflict.conflict_type, ConflictType::PathTooLong);
